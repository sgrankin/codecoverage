...
type CoverageEntry = {
  ...
  // statements holds per-block statement coverage. Present only for formats that carry
  // statement counts (Go); absent for lcov/cobertura/simplecov.
  statements?: {
    found: number  // Sum of numStmts across all blocks
    hit: number     // Sum of numStmts across blocks with hitCount > 0
    blocks: {
      key: string   // Block coordinate string, e.g. "57.54,59.16"
      count: number // numStmts for this block
      hit: number   // Max hitCount seen for this block across merged runs
    }[]
  }
}
